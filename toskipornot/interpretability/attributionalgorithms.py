...
from toskipornot.interpretability.baseline_generator import AbstractBaselineGenerator
from toskipornot.interpretability.image_interpolation import AbstractImageInterpolator

# from toskipornot.interpretability.spatial_iteration import AbstractSpatialIterator
...
class AttributionAlgorithm(ABC):
    ...
    @abstractmethod
    def execute(self, image: Tensor, idx_label: int) -> Tensor:
        pass
...
class IG_simple(AttributionAlgorithm):
    ...
    def _compute_gradients(
        ...
        grads = torch.autograd.grad(
            probs[batch_idx, channel_idx, :, :],
            image,
            grad_outputs=torch.ones_like(probs[batch_idx, channel_idx, :, :]),
        )[0].detach()
        return grads

    @staticmethod
    def _integral_approximation(
        gradients: Tensor,
        alphas: Tensor,
    ) -> Tensor:
    ...
    def execute(self, image: torch.Tensor, idx_label: int) -> Tensor:
        ...
        for idx_blending in range(blended_images.shape[0]):
            blended_grads = self._compute_gradients(
                blended_images[idx_blending, ...], 0, idx_label
            )
...
class IG_class(AttributionAlgorithm):
    ...
    def _integral_approximation(
        ...
        Returns:
            Tensor: The normalized integral approximation of the tensors.
        """
        integrated_gradients = torch.zeros(
            (gradients.shape[1:]), device=gradients.device
        )
        for i in range(gradients.shape[0] - 1):
            step_size = alphas[i + 1] - alphas[i]
            integrated_gradients += (
                step_size * 0.5 * (gradients[i, ...] + gradients[i + 1, ...])
            )
        return integrated_gradients / gradients.shape[0]
