...
def validate_model(
    ...
    with torch.no_grad():
        ...
        dice_metric.reset()
        output_folder = "./NoSkipUNet_dice_" + variation + "_seed_" + str(seed_num)
    ...
    for seed_num in range(1, 6):
        for variation in data_variations:
            ...
            config = {
                ...
                # UNet Model
                "model_type": "NoSkipUNet",  # just to keep track
                "model_params": dict(
                    spatial_dims=2,
                    in_channels=1,
                    out_channels=1,
            ...
            run = wandb.init(
                project="synthetic-texture-NoSkipUNet-dice-v6",
                name=project_name,
                config=config,
            )
            ...
            model = NoSkipUNet(**config["model_params"]).to(device)
