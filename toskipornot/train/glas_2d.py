...
def main():
    """
    Runs the training and validation loop for GlaS data.
    """
    ...
    data_dir = str(DATA_DIR / "GLaS-processed")
    ...
    for seed_num in seed_list:
        for swin_size in swin_list:
            for net in net_name:
                ...
                images = sorted(glob(os.path.join(data_dir, "train", "image", "*")))
                ...
                images = sorted(glob(os.path.join(data_dir, "valid", "image", "*")))
                masks = [im.replace("image", "label") for im in images]
                ...
                images = sorted(glob(os.path.join(data_dir, "test", "image", "*")))
                masks = [im.replace("image", "label") for im in images]
                ...
                # those filenames.
                project = "glas-" + net + "-full-v3"
                project_name = net + "_" + str(swin_size) + "_" + str(seed_num)
                output_folder = os.path.join(
                    str(CHECKPOINTS_DIR / "glas"), project_name
                )
