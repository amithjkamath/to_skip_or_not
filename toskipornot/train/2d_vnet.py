...
def validate_model(
    ...
    with torch.no_grad():
        ...
        dice_metric.reset()
        output_folder = "./vnet_dice_" + variation + "_seed_" + str(seed_num)
    ...
    for seed_num in range(1, 6):
        for variation in data_variations:
            ...
            config = {
                ...
                # VNet Model
                "model_type": "VNET",  # just to keep track
                "model_params": dict(
                    spatial_dims=2,
                    in_channels=1,
                    out_channels=1,
            ...
            run = wandb.init(
                project="synthetic-texture-vnet-dice-v6",
                name=project_name,
                config=config,
            )
            ...
            model = VNet(**config["model_params"]).to(device)
