...
def run_segmentation(model, config, images, labels, output_path, variant, seed_num):
    ...
    output_saver = SaveImage(
        output_dir=os.path.join(
            output_path,
            "Spleen_stats_" + model_name + "_" + variant + "_256_" + str(seed_num),
        ),
        output_ext=".png",
        output_postfix="out",
    )
...
def generate_plots(data_path, model_path, output_path, model_name, variant, seed_num):
    for fold in ["test"]:
        data_images = os.path.join(data_path, fold, "image")
        images = sorted(glob(os.path.join(data_images, "*")))
        data_labels = os.path.join(data_path, fold, "label")
        ...
        (
            ...
            model, config, images, labels, output_path, variant, seed_num
        )
...
if __name__ == "__main__":
    model_dir = str(CHECKPOINTS_DIR / "spleen")
    output_path = str(RESULTS_DIR / "Spleen-results")

    for variant in ["lower", "low", "in-domain", "high", "higher"]:
        for seed_num in [1, 2, 3]:
            data_path = str(DATA_DIR / "Task09_Spleen-experiment" / variant / "test")
