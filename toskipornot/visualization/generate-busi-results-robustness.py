...
def find_split(config, n_images):
    fold_state = list()
    train_indices = config["train_indices"]["value"]
    valid_indices = config["valid_indices"]["value"]
    test_indices = config["test_indices"]["value"]
    for idx in range(1, n_images + 1):
        if idx in train_indices:
            fold_state.append("train")
        elif idx in valid_indices:
            fold_state.append("valid")
