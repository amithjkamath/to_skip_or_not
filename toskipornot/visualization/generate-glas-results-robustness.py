...
def run_segmentation(model, config, images, labels, output_path, variant, seed_num):
    ...
    output_saver = SaveImage(
        output_dir=os.path.join(
            output_path,
            "GLaS_stats_" + model_name + "_" + variant + "_256_" + str(seed_num),
        ),
        output_ext=".png",
        output_postfix="out",
    )
...
if __name__ == "__main__":
    model_dir = str(CHECKPOINTS_DIR / "glas")
    output_path = str(RESULTS_DIR / "GLaS-results")
    for variant in ["lower", "low", "in-domain", "high", "higher"]:
        for seed_num in [1, 2, 3]:
            data_path = str(DATA_DIR / "GLaS-experiment" / variant / "test")
